...
class NebulaApp(MDApp):
    ...
    output_log = StringProperty("> Nebula OS v2.7 Online...\\n")
    ...
    def clear_console(self):
        self.output_log = "> Console Cleaned...\\n"
    ...
    def pip_install(self):
        ...
        def _p():
            ...
            Clock.schedule_once(lambda dt: Snackbar(text=f"{pkg} установлен").open())
        threading.Thread(target=_p).start()

    def ai_analyze(self):
        code = self.root.ids.code_input.text
        self.root.ids.ai_box.text = "AI: Код проверен. Ошибок нет." if len(code) > 10 else "AI: Напишите больше кода."
        Snackbar(text="Анализ завершен").open()

# -*- coding: utf-8: -*-
import os, sys, threading, time, json, platform, traceback, webbrowser
from io import StringIO
from kivy.lang import Builder
from kivy.clock import Clock, mainthread
from kivy.animation import Animation
from kivy.properties import StringProperty, DictProperty, ListProperty
from kivymd.app import MDApp
from kivymd.uix.button import MDFillRoundFlatButton, MDFlatButton, MDRaisedButton
from kivymd.uix.snackbar import Snackbar

# ПОЛНЫЙ ИНТЕРФЕЙС NEBULA MASTER
KV = '''
<SyntaxBtn@MDRaisedButton>:
    size_hint: None, None
    size: "75dp", "52dp"
    elevation: 6
    radius: [18, 18, 18, 18]
    md_bg_color: app.theme_cls.primary_color

MDScreenManager:
    id: screen_manager
    MDScreen:
        name: "intro"
        md_bg_color: "#000000"
        MDBoxLayout:
            orientation: "vertical"
            Widget:
            Image:
                id: serpent_intro
                source: "serpent.png"
                size_hint: None, None
                size: "400dp", "400dp"
                pos_hint: {"center_x": .5, "center_y": -.6}
            MDLabel:
                id: intro_label
                text: "NEBULA EVOLUTION"
                halign: "center"
                theme_text_color: "Custom"
                text_color: "#BB86FC"
                font_style: "H4"
                opacity: 0
            Widget:

    MDScreen:
        name: "main"
        MDBoxLayout:
            orientation: "vertical"
            md_bg_color: "#050505"
            MDTopAppBar:
                title: "Nebula Ultra [MASTER]"
                md_bg_color: "#121212"
                elevation: 12
                left_action_items: [["menu", lambda x: nav_drawer.set_state("open")]]
                right_action_items: [["robot-excited", lambda x: app.call_snake()]]

            ScrollView:
                size_hint_y: None
                height: "80dp"
                do_scroll_y: False
                MDBoxLayout:
                    adaptive_width: True
                    padding: "15dp"
                    spacing: "15dp"
                    SyntaxBtn: text: "def"; md_bg_color: "#FF5252"; on_release: app.ins("def ")
                    SyntaxBtn: text: "class"; md_bg_color: "#448AFF"; on_release: app.ins("class ")
                    SyntaxBtn: text: "torch"; md_bg_color: "#FFAB40"; on_release: app.ins("import torch\\n")
                    SyntaxBtn: text: "tg_bot"; md_bg_color: "#00E5FF"; on_release: app.ins("from telegram import Bot")
                    SyntaxBtn: text: "import"; md_bg_color: "#E040FB"; on_release: app.ins("import ")
                    SyntaxBtn: text: "async"; md_bg_color: "#FF4081"; on_release: app.ins("async ")
                    SyntaxBtn: text: ":"; md_bg_color: "#212121"; on_release: app.ins(":")

            MDBoxLayout:
                orientation: "vertical"
                padding: "15dp"
                MDCard:
                    radius: [25,]
                    md_bg_color: "#0D0D0D"
                    padding: "15dp"
                    elevation: 10
                    ScrollView:
                        MDTextField:
                            id: editor
                            text: app.code_init
                            multiline: True
                            mode: "fill"
                            on_text: app.auto_save(self.text)
                            text_color_normal: "#E1BEE7"

                MDBoxLayout:
                    size_hint_y: None
                    height: "80dp"
                    padding: [0, 15, 0, 0]
                    MDFillRoundFlatButton:
                        text: "LAUNCH SYSTEM CORE"
                        icon: "rocket-launch"
                        size_hint_x: 1
                        md_bg_color: "#6200EE"
                        on_release: app.run_engine()

            MDBottomNavigation:
                id: b_nav
                panel_color: "#121212"
                MDBottomNavigationItem:
                    name: 'ide'; text: 'IDE Editor'; icon: 'code-braces'
                MDBottomNavigationItem:
                    name: 'logs'; text: 'System Logs'; icon: 'console'
                    MDBoxLayout:
                        md_bg_color: "#000000"
                        ScrollView:
                            id: log_scroll
                            MDLabel:
                                id: console
                                text: app.log_data
                                color: "#00FFC8"
                                size_hint_y: None
                                height: self.texture_size[1]
                                padding: "20dp"
                                font_style: "Caption"

        Image:
            id: snake_helper
            source: "serpent.png"
            size_hint: None, None
            size: "300dp", "300dp"
            pos_hint: {"center_x": 1.8, "center_y": 0.35}

    MDNavigationDrawer:
        id: nav_drawer
        md_bg_color: "#121212"
        MDBoxLayout:
            orientation: "vertical"
            padding: "25dp"
            spacing: "20dp"
            MDLabel: text: "NEBULA CONTROL"; font_style: "H5"; text_color: "#BB86FC"
            MDSeparator:
            MDFlatButton: text: "TG Bridge Alpha"; icon: "bridge"; on_release: app.msg("Bridge Alpha: Online")
            MDFlatButton: text: "TG Bridge Beta"; icon: "bridge"; on_release: app.msg("Bridge Beta: Online")
            MDFlatButton: text: "Reset App State"; icon: "refresh"; on_release: app.reset_app()
            MDFlatButton: text: "Clear Logs"; icon: "trash-can-outline"; on_release: app.clear_logs()
            MDFlatButton: text: "Join Community"; icon: "telegram"; on_release: app.open_tg()
            Widget:
            MDLabel: text: "v3.3.2 Master"; font_style: "Overline"; text_color: "#444444"
'''

class NebulaApp(MDApp):
    log_data = StringProperty("> Nebula Engine Initialized...\\n")
    code_init = StringProperty("")
    vault = DictProperty({'is_fresh': True})

    def build(self):
        self.theme_cls.theme_style = "Dark"
        self.theme_cls.primary_palette = "DeepPurple"
        self.load_all_data()
        return Builder.load_string(KV)

    def on_start(self):
        # Логика разрешений Android
        if platform.system() == "android":
            from android.permissions import request_permissions, Permission
            request_permissions([
                Permission.READ_EXTERNAL_STORAGE, 
                Permission.WRITE_EXTERNAL_STORAGE, 
                Permission.INTERNET,
                Permission.MANAGE_EXTERNAL_STORAGE
            ])

        # Фирменная кат-сцена (только 1 раз)
        if self.vault.get('is_fresh', True):
            s = self.root.ids.serpent_intro
            l = self.root.ids.intro_label
            # Анимация змея
            anim = Animation(pos_hint={"center_x": .5, "center_y": .5}, duration=3, t='out_back')
            anim.bind(on_complete=self.finish_intro)
            anim.start(s)
            # Плавное появление текста
            Animation(opacity=1, duration=2.5).start(l)
            
            # Сохраняем состояние
            self.vault['is_fresh'] = False
            self.save_all_data()
        else:
            self.root.ids.screen_manager.current = "main"

    def finish_intro(self, *args):
        Clock.schedule_once(lambda x: setattr(self.root.ids.screen_manager, 'current', 'main'), 1)

    def run_engine(self):
        self.root.ids.b_nav.switch_tab('logs')
        code = self.root.ids.editor.text
        
        def execution_thread():
            backup_stdout = sys.stdout
            result_io = sys.stdout = StringIO()
            start_t = time.time()
            try:
                # Позволяем коду видеть функции приложения
                exec(code, {**globals(), 'app': self})
                output = result_io.getvalue().strip() or "--- Done (No Output) ---"
            except Exception:
                output = traceback.format_exc()
            finally:
                sys.stdout = backup_stdout
            
            self.post_to_console(f"{output}\\n[Runtime: {round(time.time()-start_t, 3)}s]")

        threading.Thread(target=execution_thread, daemon=True).start()

    @mainthread
    def post_to_console(self, text): 
        self.log_data += f"\\n[{time.strftime('%H:%M:%S')}] {text}"
        Clock.schedule_once(lambda x: setattr(self.root.ids.log_scroll, 'scroll_y', 0), 0.1)

    def auto_save(self, text):
        path = os.path.join(self.user_data_dir, "nebula_session.py")
        with open(path, "w", encoding='utf-8') as f: f.write(text)

    def load_all_data(self):
        # Загрузка Vault (Intro state)
        v_p = os.path.join(self.user_data_dir, "vault.json")
        if os.path.exists(v_p):
            with open(v_p, "r") as f: self.vault = json.load(f)
        
        # Загрузка последнего кода
        c_p = os.path.join(self.user_data_dir, "nebula_session.py")
        if os.path.exists(c_p):
            with open(c_p, "r") as f: self.code_init = f.read()
        else:
            self.code_init = "import torch\\nprint('Nebula Engine Online')"

    def save_all_data(self):
        v_p = os.path.join(self.user_data_dir, "vault.json")
        with open(v_p, "w") as f: json.dump(dict(self.vault), f)

    def ins(self, t): self.root.ids.editor.insert_text(t)
    def msg(self, t): Snackbar(text=t).open()
    def clear_logs(self): self.log_data = "> Logs Purged\\n"
    def open_tg(self): webbrowser.open("https://t.me/nebula_evolution")
    def reset_app(self): 
        self.vault['is_fresh'] = True
        self.save_all_data()
        self.msg("Restart required to see Intro")
        
    def call_snake(self): 
        s = self.root.ids.snake_helper
        Animation(pos_hint={"center_x": .82, "center_y": .38}, duration=1.2, t='out_elastic').start(s)
        self.msg("Serpent Assistant Active")

if __name__ == "__main__":
    NebulaApp().run()
  
